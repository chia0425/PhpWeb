...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        device_requests = [2, 2]  # two devices, each 2U

        for i, size in enumerate(device_requests, start=1):
            result = allocate_best_fit(size, datacenter_name="TestDC")
            if result:
                allocated_range = (result['start_ru'], result['start_ru'] + result['length'] - 1)
                print(f"\nDevice {i} allocated: {result['rack']} RUs {allocated_range}")
            else:
                print(f"\nDevice {i} allocation failed (no suitable free block)")

            # Show current free blocks after each allocation
            print(f"Free slots in rack {rack.name} after device {i}:")
